...
function ComputerNetwork() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('computerNetwork');
  ...
  const handleAdd = () => {
    if (linkName && linkUrl) {
      ...
      setLinks(updated);
      localStorage.setItem('computerNetwork', JSON.stringify(updated));
      setLinkName('');
      setLinkUrl('');
      setShowForm(false);
    }
  ...
  const handleReset = () => {
    setLinks([]);
    localStorage.setItem('computerNetwork', JSON.stringify([]));
  };
