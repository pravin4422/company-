...
function OperatingSystem() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('operatingSystem');
  ...
  const handleAdd = () => {
    if (linkName && linkUrl) {
      ...
      setLinks(updated);
      localStorage.setItem('operatingSystem', JSON.stringify(updated));
      setLinkName('');
      setLinkUrl('');
      setShowForm(false);
    }
  ...
  const handleReset = () => {
    setLinks([]);
    localStorage.setItem('operatingSystem', JSON.stringify([]));
  };
