...
function SQL() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const defaultLinks = [
        { id: 1, name: 'SQLZoo Solutions', url: 'https://github.com/ritakalach/sqlzoo-solutions' }
      ];
      setLinks(defaultLinks);
      localStorage.setItem('sql', JSON.stringify(defaultLinks));
    }
  }, []);
  ...
  const handleAdd = () => {
    if (linkName && linkUrl) {
      ...
      setLinks(updated);
      localStorage.setItem('sql', JSON.stringify(updated));
      setLinkName('');
      setLinkUrl('');
      setShowForm(false);
    }
